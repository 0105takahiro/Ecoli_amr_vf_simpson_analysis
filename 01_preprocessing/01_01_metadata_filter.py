...
# Extract year from 'Collection_Date' field
...
# Build 'Assembly accession'
ref = metadata_df["RefSeq_ID"].astype("string").str.strip()
gca = metadata_df["GenBank_ID"].astype("string").str.strip()
metadata_df["Assembly accession"] = ref.where(ref.str.startswith("GCF_", na=False), gca)

# Remove the original accession columns
metadata_df.drop(columns=["GenBank_ID", "RefSeq_ID"], inplace=True, errors="ignore")

col = metadata_df.pop("Assembly accession")
metadata_df.insert(0, "Assembly accession", col)

metadata_df=metadata_df.sort_index()

# Save filtered metadata to CSV
metadata_df.to_csv(OUTPUT_DIR / "ecoli_genomes_25875_metadata.csv")
