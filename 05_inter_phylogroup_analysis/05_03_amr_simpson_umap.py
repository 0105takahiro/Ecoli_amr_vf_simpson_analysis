...
AMR_CSV = ROOT / "output" / "02_gene_screening" / "amr_and_vf_genes" / "amr_genes_presence_absence.csv"
AMR_DISTANCE_MATRIX_CSV = ROOT / "output" / "03_distance" / "genotype_distance" / "genotypic_distance_matrix_all_phylogroups" / "amr_simpson_distance_matrix_all_phylogroups.csv"
...
ax.vlines(x=7.7, ymin=ymin-ylength*0.005, ymax=ymax+ylength*0.005, ls="--", lw=0.12, colors="grey")
ax.vlines(x=14 , ymin=ymin-ylength*0.005, ymax=ymax+ylength*0.005, ls="--", lw=0.12, colors="grey")
ax.hlines(y=15.8, xmin=14, xmax=xmax+xlength*0.005, ls="--", lw=0.12, colors="grey")
...
plt.savefig(FIGURE_OUTPUT_DIR/"umap_amr_simpson.pdf",
            dpi=600,
            bbox_inches="tight",
            pad_inches=0.0
            )
...
def pie_plot(cluster_num: int)-> None:
    ...
    plt.savefig(  FIGURE_OUTPUT_DIR /f"pie_chart_for_cluster{cluster_num}_amr_simpson.pdf", dpi=600, bbox_inches="tight", pad_inches=0)
