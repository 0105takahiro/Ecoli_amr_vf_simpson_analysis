...
VF_DISTANCE_MATRIX_CSV = ROOT / "output" / "03_distance" / "genotype_distance" / "genotypic_distance_matrix_all_phylogroups" / "vf_simpson_distance_matrix_all_phylogroups.csv"
...
ax.vlines(x=xmin+0.4, ymin=ymin+0.5, ymax=2.5, ls='--', lw=0.12, colors='grey')
ax.vlines(x=17, ymin=ymin+0.5, ymax=2.5, ls='--', lw=0.12, colors='grey')
ax.hlines(y=ymin+0.5, xmin=xmin+0.4, xmax=17, ls='--', lw=0.12, colors='grey')
ax.hlines(y=2.5, xmin=xmin+0.4, xmax=17, ls='--', lw=0.12, colors='grey')

ax.vlines(x=34, ymin=5, ymax=ymax-0.3, ls='--', lw=0.12, colors='grey')
ax.vlines(x=xmax-0.3, ymin=5, ymax=ymax-0.3, ls='--', lw=0.12, colors='grey')
ax.hlines(y=5, xmin=34, xmax=xmax-0.3, ls='--', lw=0.12, colors='grey')
ax.hlines(y=ymax-0.3, xmin=34, xmax=xmax-0.3, ls='--', lw=0.12, colors='grey')
...
plt.savefig(
    FIGURE_OUTPUT_DIR / "umap_vf_simpson_1st.pdf",
    dpi=600,
    bbox_inches='tight',
    pad_inches=0.0
    )
...
def pie_plot(cluster_num, cluster_name):
    ...
    OUT_PATH = FIGURE_OUTPUT_DIR / f"pie_chart_for_cluster_{cluster_name}_vf_simpson.pdf"
...
# ============================================================
vf_distance_matrix_non_b2 = vf_distance_matrix.loc[cluster1.index, cluster1.index]

mapper_2nd = umap.UMAP(random_state=0, n_neighbors=20, metric='precomputed',
                    n_components=2, min_dist=0.25)
embedding_2nd = mapper_2nd.fit_transform(vf_distance_matrix_non_b2)

embedding_df_2nd = pd.DataFrame(embedding_2nd, index=cluster1.index, columns=['UMAP1','UMAP2'])
embedding_merged_2nd = pd.merge(embedding_df_2nd, phylogroup_df, how='inner', left_index=True, right_index=True).dropna()

# Scatter plot for cluster 1 (second embedding)
plt.figure(figsize=(9, 6))
ax = sns.scatterplot(
    data=embedding_merged_2nd,
    x='UMAP1', y='UMAP2',
    s=2.5, hue='Phylogroup',
    hue_order=['A','B1','B2','C','D','E','F','G'],
    palette=custom_palette,
...
# Save figure
plt.savefig(FIGURE_OUTPUT_DIR / "umap_vf_simpson_2nd.pdf", dpi=600, bbox_inches='tight')
