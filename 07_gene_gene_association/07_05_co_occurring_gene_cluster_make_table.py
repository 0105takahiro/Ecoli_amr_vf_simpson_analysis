...
INPUT_DIR = ROOT / "output" /  "07_gene_gene_association" /"co_occurring_gene_clusters"
...
fig_height = 0.28 * (len(amr_data) + len(vf_data))
fig, ax = plt.subplots(
    2, 1, sharex=False,
...
ax[0].set_yticklabels(amr_labels, fontsize=8)
ax[1].set_yticklabels(vf_labels, fontsize=8)

# Ticks styling
for a in ax:
    a.tick_params(pad=1, axis='x')
    a.tick_params(axis='x', which='major', labelsize=12,
                  labelbottom=False, bottom=False, top=False, labeltop=True)
    a.tick_params(axis='y', which='major', labelsize=8.5,
                  labelbottom=False, bottom=False, top=False, labeltop=True)

# Right-side "representative gene" labels
for idx, rep in enumerate(represent_amr):
    ax[0].text(rep_x, idx + 0.5, rep, fontsize=10.5, ha='left', va='center',
               transform=ax[0].transData)
for idx, rep in enumerate(represent_vf):
    ax[1].text(rep_x, idx + 0.5, rep, fontsize=10.5, ha='left', va='center',
...
ax[0].text(shade_x + 0.05, -0.4, "Representative gene", fontsize=12,
           ha='left', va='bottom', transform=ax[0].transData)
ax[1].text(shade_x + 0.05, -0.4, "Representative gene", fontsize=12,
           ha='left', va='bottom', transform=ax[1].transData)

ax[0].text(-0.8, -0.6, "AMR", fontsize=14, ha='center', va='bottom',
           transform=ax[0].transData)
ax[1].text(-0.8, -0.6, "VF",  fontsize=14, ha='center', va='bottom',
           transform=ax[1].transData)

fig.subplots_adjust(hspace=0.3, left=0.2, right=0.85)
