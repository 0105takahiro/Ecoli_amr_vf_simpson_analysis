...
INPUT_DIR_2  = ROOT / "output" / "genotypic_distance_matrix_all_phylogroups"

# Output: distance matrices split by each phylogroup
OUTPUT_DIR = ROOT / "output" / "genotypic_distance_matrix_each_phylogroup"
...
def export_phylogroup_distance_matrices(
    ...
    for g in groups:
        ...
        out_path = os.path.join(output_dir, f"{gene.lower()}_{metric.lower()}_distance_{g.lower()}.csv")
        sub.to_csv(out_path)
