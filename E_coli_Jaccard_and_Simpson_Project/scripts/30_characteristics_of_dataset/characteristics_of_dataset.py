...
OUTPUT_DIR = ROOT / "output" / "fundamental_results"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

FILTERED_FILES_CSV = INPUT_DIR / "preparation"/ "ecoli_genomes_filtered_25080.csv"

METADATA_CSV = INPUT_DIR / "preparation"/ "ecoli_genomes_26789_metadata.csv"
ASSEMBLY_STATS_CSV = INPUT_DIR / "preparation" / "ecoli_genomes_26789_assembly_stats.csv"
...
OUT_FIGURE = FIGURE_OUTPUT_DIR / "amr_vf_genes_boxplot_combined_horizontal.pdf"
plt.rcParams['font.family'] = 'Arial'

fig, axs = plt.subplots(1, 2, figsize=(5.2, 2.5), sharex=True, gridspec_kw={'wspace': 0.45})

# Plot A: AMR
sns.boxplot(x='Phylogroup', y='No. of AMR genes', data=amr_counts_df,
            order=phylogroup_order, palette=palette, width=0.7, ax=axs[0], fliersize=0.08)
axs[0].set_ylabel('No. of AMR genes', fontsize=11)
axs[0].set_xlabel('')
...
sns.boxplot(x='Phylogroup', y='No. of VF genes', data=vf_counts_df,
            order=phylogroup_order, palette=palette, width=0.7, ax=axs[1], fliersize=0.08)
axs[1].set_ylabel('No. of VF genes', fontsize=11)
axs[1].set_xlabel('')
...
fig.supxlabel('Phylogroup', fontsize=13, y=-0.08)
fig.text(0.04, 0.9, 'a', fontsize=15, fontweight='bold')
fig.text(0.49, 0.9, 'b', fontsize=15, fontweight='bold')
