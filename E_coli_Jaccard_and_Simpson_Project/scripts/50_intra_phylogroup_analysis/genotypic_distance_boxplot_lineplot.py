...
fig, axs = plt.subplots(2, 1, figsize=(6.7, 3.15),
                        sharex=True,
...
for pos_pg, label in zip(phylogroup_positions, phylogroups):
    ax_amr.text(pos_pg, 1.04, label, ha='center', va='bottom', fontsize=10, transform=ax_amr.get_xaxis_transform())
...
box_vf = ax_vf.boxplot(
    ...
    patch_artist=True,
    widths=0.13,
    flierprops=dict(marker='o', markersize=0.8, linestyle='none'),
...
# --- Save figure ---
fig.savefig(OUT_FIGURE_BOXPLOT, bbox_inches='tight', dpi=600)
...
# Set panel titles
g.axes[0][0].set_title("AMR", fontsize=9, pad=4)
g.axes[0][1].set_title("VF", fontsize=9, pad=4)
...
g._legend.set_title("Phylogroup", prop={'size': 9})
